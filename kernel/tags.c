...
int find_param_tag(const char *name, char** b)
{
	char *data_buf;
	unsigned int item_num;
	struct tags_item_t *item;
...
int get_param_data(const char *name, char *buf, unsigned int buflen)
{
	...
	len = find_param_tag(name, &data_buf);
...
int set_param_data(const char *name, char *buf, int buflen)
{
	char *data_buf;
	int len;
