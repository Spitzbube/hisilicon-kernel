...
static void hi_mci_ctrl_power(struct himci_host *host, unsigned int flag, int c)
{
	himci_trace(2, "begin"); //137

	if ((flag != host->Data_100) || (c == 1))
	{
		if (flag == POWER_OFF)
...
static void hi_mci_init_card(struct himci_host *host)
{
	...
	hi_mci_ctrl_power(host, POWER_OFF, 1/*TODO*/);
	/* card power on */
	hi_mci_ctrl_power(host, POWER_ON, 1/*TODO*/);
...
static void hi_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_OFF:
		hi_mci_ctrl_power(host, POWER_OFF, 0/*TODO*/);
		break;
	case MMC_POWER_UP:
	case MMC_POWER_ON:
		hi_mci_ctrl_power(host, POWER_ON, 0/*TODO*/);
		break;
	}
...
static int /*__devexit*/ __exit hi_mci_remove(struct platform_device *pdev)
{
	struct mmc_host *mmc = platform_get_drvdata(pdev);

	himci_trace(2, "begin"); //1210
	himci_assert(pdev);
	...
	if (mmc) {
		...
		mmc_remove_host(mmc);
		hi_mci_ctrl_power(host, POWER_OFF, 0);
		hi_mci_control_cclk(host, DISABLE);
		iounmap(host->base);
