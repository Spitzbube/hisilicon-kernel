...
#warning TODO: VDP_VID_SetLayerAddr
    ...
    if(u32Chan == 0)
    {
        VDP_RegWrite((HI_U32)(&(pVdpReg->V0_P0LADDR.u32) + u32Data * VID_OFFSET), u32LAddr); 
        VDP_RegWrite((HI_U32)(&(pVdpReg->V0_P0CADDR.u32) + u32Data * VID_OFFSET), u32CAddr); 
    }
    else if(u32Chan == 1)
    {
...
#warning TODO
    ...
    volatile U_VP0_ACCMLN     VP0_ACCML[3];
    volatile U_VP0_ACCMHN    VP0_ACCMH[3];
    HI_U32 ii = 0;
...
//    if(u32Data >= VP_MAX)
//    {
//        HI_PRINT("Error,VDP_VP_SetAccWeightAddr() Select Wrong VP ID\n");
//        return ;
//    }
//
//    volatile U_ACCAD ACCAD;
//    
//    ACCAD.bits.coef_addr = uAccAddr;
//    VDP_RegWrite((HI_U32)(&(pVdpReg->ACCAD.u32)+ u32Data * VP_OFFSET), ACCAD.u32);
//}
...
{
    volatile U_DHD0_DITHER0_CTRL DHD0_DITHER0_CTRL;
    volatile U_DHD0_DITHER1_CTRL DHD0_DITHER1_CTRL;
...
{
    volatile U_DHD0_DITHER0_COEF0 DHD0_DITHER0_COEF0;
    volatile U_DHD0_DITHER0_COEF1 DHD0_DITHER0_COEF1;
    volatile U_DHD0_DITHER1_COEF0 DHD0_DITHER1_COEF0;
    volatile U_DHD0_DITHER1_COEF1 DHD0_DITHER1_COEF1;
...
{ 
   ...
   HI_PRINT("regsize = %d\n",regsize);
    ...
    printk ("the register has been writen now !\n\n" );
    i = 0;
    ...
    {  
       ...
       //     rdata = RegRead(testaddr[j]); //because in DUT, reserved bit is writeable
        ...
        if (rdata == cdata)
        {
        //    printk ("Right! Register Address %x is right! :)\n", (testaddr[j]));
        //    printk ("Register Data is 0x %x\n", rdata);
        }
        else
        {
            counter ++ ;
        //    printk ("Error! Register Address  %x is wrong! :(\n", (testaddr[j]));
        //    printk ("the right Data is %x\n", cdata);
        //    printk ("the wrong Data is %x\n", rdata);
    ...
    if(counter == 0)
    {
        printk ("Right! All the register are right!\n");
    //    cout << "Right! All the register are right!" << endl;
    }
    else
    {
        printk ("Error! We find %d wrong register. Please check it!\n\n",counter);
    }
   ...
   HI_PRINT("regsize = %d\n",regsize);
    ...
    {  
        ...
        else
        {
            counter ++ ;
            printk ("Error! Register Address  %x is wrong! :(\n", (testaddr[j]));
            printk ("the Reg def Data is %x\n", cdata);
            printk ("the Actual reg Data is %x\n", rdata);
...
HI_VOID VDP_DATE_ResetFmt(DISP_VENC_E eDate, HI_DRV_DISP_FMT_E enFmt)
{
    ...
    if (DISP_VENC_SDATE0 == eDate)
    {
        ...
        /*zkun_confirm:*/
        VDP_DATE_SetSrcCoef(eDate, u32aSDDATEDefSrcCoef);
...
//change default config for better P-P value
...
#if 0
    /*    
    0xF8CC0120  0x00004000 //common block(29\14) iso���ƼĴ���Ĭ��Ϊ0��
...
HI_VOID VDP_VDAC_SetLink(DISP_VENC_E eDate, HI_U32 uVdac, HI_DRV_DISP_VDAC_SIGNAL_E signal)
{
    ...
    //printk("VDP_VDAC_SetLink date=%d, dac=%d, sig=%d\n", eDate, uVdac, signal);
