...
/* Synm Iop  Itf  Vact Vbb Vfb   Hact  Hbb Hfb      Bvact Bvbb Bvfb  Hpw Vpw Hmid bIdv bIhs bIvs */
...
#endif
  //480I: HDMI���Ҫ��hmid=310, ��YPbPrҪ��hmid=0, 
  //����һ���û�����ʹ��HDMI���480I�����Բ�֧��HDMI_480I�����ѡ��hmid=0
 ...
 { {1,   1,   2,   2160,  82,  8,   4096, 384, 1020,     1,  1,  1,    88, 10,  1, 0, 0,  0},/* 480I(NTSC) */
   DISP_CLOCK_SOURCE_HD0,
   {0x12000000, 0x02002063}, // 1080i50
   {HI_DRV_DISP_FMT_4096X2160_24,    DISP_STEREO_NONE, HI_FALSE,  {0,0,3840,2160} , {0,0,3840,2160} ,{16,9},  2400, HI_DRV_CS_BT709_YUV_LIMITED},
 },
};
...
static HI_VOID internal_set_extlineRegister(HI_U32 u32LayerId, HI_S32 s32exl)
{
    ...
    VDP_VID_SetDrawMode(u32LayerId, extRegValue);
                            ...
                            ALG_VZME_RTL_PARA_S *stZmeO)
{
    ...
    /*
     *cv200, wbc-dhd0 and all video layers are 4 taps,
     *so pfVZmeVdpSQSet should be used.
     */
    pfUA->pfVZmeVdpSQSetSeperateAddr(stZmeI, stZmeO);    
    return HI_SUCCESS;	
}
...
HI_S32 Chip_Specific_SetWbc3DInfo(DISP_WBC_E eWbc, HI_DISP_DISPLAY_INFO_S *pstDispInfo, HI_RECT_S *in)
{
    ...
    if (DISP_STEREO_SBS_HALF == pstDispInfo->eDispMode)
    {
        stRect.u32DXL = (((HI_U32)in->s32Width / 2) & 0xfffffffc) - 1;
        /*wbc 2d write back.*/
        VDP_WBC_SetThreeMd(VDP_LAYER_WBC_HD0, 0);
    }
    else if (DISP_STEREO_TAB == pstDispInfo->eDispMode)
    {
        stRect.u32DXL = (((HI_U32)in->s32Height / 2) & 0xfffffffe) - 1;                
        /*wbc 2d write back.*/
        VDP_WBC_SetThreeMd(VDP_LAYER_WBC_HD0, 0);
    }
    else if (DISP_STEREO_FPK == pstDispInfo->eDispMode)
    {
        /*fpk only write left eye.*/
...
HI_S32 Chip_Specific_WinHalSetAddr(HI_U32 u32LayerId, WIN_HAL_PARA_S *pstPara, HI_S32 s32exl)
{
    ...
    HI_U32 SrcAddrL[2], SrcAddrC[2], OffsetL, OffsetC, StreamOffsetL, StreamOffsetC;
    ...
    if (  (pstPara->pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV12)
        ...
        OffsetL = (pstPara->stIn.s32Y * pstAddr[0].u32Stride_Y);
        OffsetC = (pstPara->stIn.s32Y * pstAddr[0].u32Stride_Y/2);
    } else if (  (pstPara->pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV12_CMP)
        ||(pstPara->pstFrame->ePixFormat == HI_DRV_PIX_FMT_NV21_CMP)
        ) { 

        SrcAddrL[0]  = pstAddr[0].u32PhyAddr_YHead;
        SrcAddrL[1]  = pstAddr[1].u32PhyAddr_YHead;
        
        SrcAddrC[0]  = pstAddr[0].u32PhyAddr_CHead;
        SrcAddrC[1]  = pstAddr[1].u32PhyAddr_CHead;
        
        /*for compress mode, we should calculate 2 results:
          1) StreamOffsetL/C: distance from the cropped compress header addr to cropped stream
                       location;
          2) OffsetL/C: distance from the original compress header to cropped compress header.
          */          
        OffsetL = (pstPara->stIn.s32Y * 16);
        OffsetC = (pstPara->stIn.s32Y * 16/2);
        
        StreamOffsetL = (pstPara->stIn.s32Height - pstPara->stIn.s32Y) * 16
                         + (pstPara->stIn.s32Y * pstAddr[0].u32Stride_Y);
        
        StreamOffsetC = ((pstPara->stIn.s32Height - pstPara->stIn.s32Y) * 16
                         + (pstPara->stIn.s32Y * pstAddr[0].u32Stride_Y)) / 2;

        /*we should configure decompress addr.*/
        VDP_VID_SetDcmpOffset(u32LayerId,  StreamOffsetL, StreamOffsetC);        
    } else{        
       DISP_FATAL_RETURN();
    }
    ...
    VDP_VID_SetInCrop(u32LayerId, pstPara->stIn.s32X, 
                        pstPara->stIn.s32X + pstPara->stIn.s32Width);
    
    /*temerary modification, for pip, turn off v1/v4 multimode*/
    VDP_VID_SetMultiModeEnable(1, 0);
    VDP_VID_SetMultiModeEnable(4, 0);
    ...
    if ((pstPara->en3Dmode == DISP_STEREO_FPK)||
        ...
        if (HI_DRV_FT_NOT_STEREO == pstPara->pstFrame->eFrmType){
            VDP_VID_SetLayerAddr(u32LayerId, 0, SrcAddrL[0] + OffsetL, 
                                                SrcAddrC[0] + OffsetC, 
                                                pstAddr[0].u32Stride_Y + OffsetL, 
                                                pstAddr[0].u32Stride_C + OffsetC);
            
            VDP_VID_SetLayerAddr(u32LayerId, 1, SrcAddrL[0] + OffsetL, 
                                                SrcAddrC[0] + OffsetC, 
                                                pstAddr[0].u32Stride_Y + OffsetL, 
                                                pstAddr[0].u32Stride_C + OffsetC);
            ...
            if (pstPara->bRightEyeFirst) {
                ...
                VDP_VID_SetLayerAddr(u32LayerId, 0, SrcAddrL[1] + OffsetL,
                                                    SrcAddrC[1] + OffsetC, 
                ...
                VDP_VID_SetLayerAddr(u32LayerId, 1, SrcAddrL[1] + OffsetL,
                                                    SrcAddrC[1] + OffsetC, 
                                                    pstAddr[1].u32Stride_Y, 
                                                    pstAddr[1].u32Stride_C);
            }
