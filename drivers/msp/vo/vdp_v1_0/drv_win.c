...
HI_S32 DRV_WIN_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    */
    ...
    PROC_PRINT(p, "%-19s:%-20s\n", "|Circumrotate", g_pWinTrueString[pstNewFrame->u32Circumrotate > 0]);
    ...
    PROC_PRINT(p, "%-19s:%-4d:%-15d", "CustAspectRatio", (HI_U32)pstProcInfo->stAttr.stCustmAR.u8ARw,
    ...
    PROC_PRINT(p, "%-19s:%d/%d/%d/%d\n", "|OriRect(X/Y/W/H)",  pstPriv->stOriginImageRect.s32X,
...
{
    HI_PRINT("------------------Win Debug Option----------------\n");
    HI_PRINT("echo help                    > /proc/msp/winXXXX  \n");
    HI_PRINT("echo pause   on/off          > /proc/msp/winXXXX  \n");
    HI_PRINT("echo reset   black/still     > /proc/msp/winXXXX  \n");
    HI_PRINT("echo freeze  black/still/off > /proc/msp/winXXXX  \n");
    HI_PRINT("echo order   up/down         > /proc/msp/winXXXX  \n");
    HI_PRINT("echo quick   on/off          > /proc/msp/winXXXX  \n");
    HI_PRINT("echo rota    0/90/180/270    > /proc/msp/winXXXX  \n");
    HI_PRINT("echo flip    hori/vert       > /proc/msp/winXXXX  \n");
    HI_PRINT("echo depth   X               > /proc/msp/winXXXX  \n");
    HI_PRINT("echo capture path            > /proc/msp/winXXXX  \n");
...
HI_S32 WinProcCmdProcee(HI_HANDLE hWin, HI_CHAR *pArg1,HI_CHAR *pArg2)
{
    ...
    else if (0 == HI_OSAL_Strncmp(pArg1, "depth", strlen("depth")))
    {
#if 0
        HI_S32 s32Depth = 0;
...
HI_S32 Make_CaptuerFrame(HI_DRV_VIDEO_FRAME_S *pFrameinfo,HI_DRV_VIDEO_FRAME_S *pCapFrameinfo)
{
    ...
    HI_U32 u32StillFrameStartVirAddr,u32FrameStartVirAddr;
    ...
    u32FrameStartVirAddr =(HI_U32) phys_to_virt (u32FrameStartPhyAddr);
...
HI_S32 HI_DRV_WIN_CapturePicture(HI_HANDLE hWin, HI_DRV_VIDEO_FRAME_S *pstPic)
{
    ...
    Make_CaptuerFrame(&(stCapture.CapPicture),pstPic);
...
//    .pfnWin3DMode      = HI_DRV_WIN_Set3DMode,
    ...
    .pfnWinUpdatePqData = HI_DRV_WIN_UpdatePqData,    
};
