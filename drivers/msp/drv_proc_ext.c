...
{
    DRV_PROC_ITEM_S *item = PDE_DATA(inode); /*PDE(inode)->data;*/
...
{
	/*HI_PRINT*/HI_DRV_PROC_EchoHelper("\nPLS type \"echo 0xxxxxxxxx 0xxxxxxxxx > /proc/msp/demux\"\n");
	/*HI_PRINT*/HI_DRV_PROC_EchoHelper("E.g.: \"echo 0x40002 0x4010 > /proc/msp/demux\"\n");
}
    ...
    const char __user *buf, size_t count, loff_t *ppos, PROC_CTRL fun_ctl)
{
    ...
    szBuffer[/*count -1*/ (count > 1)? count-1: 0] = '\0';
