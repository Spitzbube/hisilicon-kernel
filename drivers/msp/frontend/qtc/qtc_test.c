...
{
    HI_U32 u32QtcRegAddr = (HI_U32)IO_ADDRESS(D_QTC_PHY_BASE_REG_ADDR);
    HI_U32 u32QtcPhyBuf, u32QtcVirBuf;
    ...
    u32QtcVirBuf = stMmzBuf.u32StartVirAddr;
    HI_INFO_QTC("u32QtcPhyBuf = 0x%x\n", u32QtcPhyBuf);
    HI_INFO_QTC("u32QtcVirBuf = 0x%x\n", u32QtcVirBuf);
    HI_INFO_QTC("u32Times = 0x%x\n", u32Times);
    
    local_irq_save(u64IrqFlags);
    *(volatile HI_U32 *)(u32QtcRegAddr + 0x44) = 0;/*status reg : write 0*/


    *(volatile HI_U32 *)(u32QtcRegAddr + 0x40) = 1;/*control reg : write 1, start*/
    //for(time_out = 0; time_out < 200; time_out++)
    {
        test_2ddr_burst8((unsigned int *)u32QtcRegAddr, (unsigned int *)u32QtcVirBuf, u32Times);
        //u32_ADC_data[time_out] = *(volatile HI_U32 *)(u32_VirQTC_PHY_BASE_ADDR + 0x00); /*read data*/
    }
    u32QtcStatus = *(volatile HI_U32 *)(u32QtcRegAddr + 0x44);
    
    *(volatile HI_U32 *)(u32QtcRegAddr + 0x40) = 0;/*control reg : write 0, stop*/
    ...
    *(volatile HI_U32 *)(u32QtcRegAddr + 0x44) = 0;/*status reg : write 0*/
    ...
    for(i = 0; i < 0x20; i++)
    {
        HI_INFO_QTC("0x%x\n", *(((HI_U32 *)u32QtcVirBuf) + i));
    }
    HI_INFO_QTC("L%d\n", __LINE__);
    SV_klib_fwrite((HI_S8*)u32QtcVirBuf, D_QTC_DATA_SIZE, p_file_qtc);
    HI_INFO_QTC("L%d\n", __LINE__);
...
static HI_VOID SocQtcTc_0_0_3(HI_VOID)
{
    ...
    u32QtcRegAddr = (HI_U32)IO_ADDRESS(D_MV300_QTC_PHY_BASE_REG_ADDR);
    D_SVPrintTc(); 
    ...
    u32QtcVirBuf = stMmzBufMv300.u32StartVirAddr;
    HI_INFO_QTC("u32QtcPhyBuf = 0x%x\n", u32QtcPhyBuf);
    HI_INFO_QTC("u32QtcPhyEnd = 0x%x\n", u32QtcPhyBuf + D_QTC_DATA_SIZE);
    HI_INFO_QTC("u32QtcVirBuf = 0x%x\n", u32QtcVirBuf);
    
    local_irq_save(u64IrqFlags);
    *(volatile HI_U32 *)(u32QtcRegAddr + 0x48) = u32QtcPhyBuf;/*buffer start addr*/
    *(volatile HI_U32 *)(u32QtcRegAddr + 0x4c) = u32QtcPhyBuf + D_QTC_DATA_SIZE;/*buffer end addr*/
    
    u32QtcContrl = *(volatile HI_U32 *)(u32QtcRegAddr + 0x40);/*read contrl reg*/
    u32QtcContrl = u32QtcContrl | 0x7;/*collect data*/
    *(volatile HI_U32 *)(u32QtcRegAddr + 0x40) = u32QtcContrl;
    ...
    if(u32QtcStatus & 0x1)
    {
        HI_INFO_QTC("******u32QtcStatus = 0x%x******\n", u32QtcStatus);
         /* get QAM adc data */
        p_file_qtc = SV_klib_fopen("./qtc.bin", O_RDWR | O_CREAT/* |O_APPEND*/ , 00777);
        SV_klib_fwrite((HI_S8*)stMmzBufMv300.u32StartVirAddr, D_QTC_DATA_SIZE, p_file_qtc);
        HI_INFO_QTC("L%d\n", __LINE__);
        SV_klib_fclose(p_file_qtc);
        HI_INFO_QTC("L%d\n", __LINE__);
    }
...
static HI_VOID __exit SocQtcTest_ModExit(HI_VOID)
{
     HI_CHIP_TYPE_E enChipType;
    HI_U32 u32ChipVersion = 0;
