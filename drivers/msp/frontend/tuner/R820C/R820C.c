...
UINT8 R828_iniArry[27] = {0x83, 0x32, 0x75, 0x80, 0x00, 0xD6, 0x6C, 0xF5, 0x63,
...
UINT8 R828_Arry[27];
R828_SectType IMR_Data[MAX_TUNER_NUM][5]; //Please keep this array data for standby mode.
I2C_TYPE  R828_I2C;
...
UINT8  R828_IMR_done_flag[MAX_TUNER_NUM] = {FALSE, FALSE, FALSE, FALSE};
UINT8  R828_Fil_Cal_flag[MAX_TUNER_NUM][STD_SIZE];
static UINT8 R828_Fil_Cal_code[MAX_TUNER_NUM][STD_SIZE];

static UINT8 Xtal_cap_sel[MAX_TUNER_NUM];
static UINT8 Xtal_cap_sel_tmp[MAX_TUNER_NUM];
//----------------------------------------------------------//
...
//                   Internal Functions                     //
//----------------------------------------------------------//
R828_ErrCode R828_Xtal_Check(R828_TUNER_NUM_TYPE R828_Tuner_Num);
R828_ErrCode R828_InitReg(R828_TUNER_NUM_TYPE R828_Tuner_Num);
R828_ErrCode R828_IMR_Prepare(R828_TUNER_NUM_TYPE R828_Tuner_Num);
R828_ErrCode R828_IMR(R828_TUNER_NUM_TYPE R828_Tuner_Num, UINT8 IMR_MEM, BOOL IM_Flag);
R828_ErrCode R828_PLL(R828_TUNER_NUM_TYPE R828_Tuner_Num, UINT32 LO_Freq, R828_Standard_Type R828_Standard);
R828_ErrCode R828_MUX(R828_TUNER_NUM_TYPE R828_Tuner_Num, UINT32 RF_KHz);
R828_ErrCode R828_IQ(R828_TUNER_NUM_TYPE R828_Tuner_Num, R828_SectType* IQ_Pont);
R828_ErrCode R828_IQ_Tree(R828_TUNER_NUM_TYPE R828_Tuner_Num, UINT8 FixPot, UINT8 FlucPot, UINT8 PotReg, R828_SectType* CompareTree);
R828_ErrCode R828_CompreCor(R828_TUNER_NUM_TYPE R828_Tuner_Num, R828_SectType* CorArry);
R828_ErrCode R828_CompreStep(R828_TUNER_NUM_TYPE R828_Tuner_Num, R828_SectType* StepArry, UINT8 Pace);
R828_ErrCode R828_Muti_Read(R828_TUNER_NUM_TYPE R828_Tuner_Num, UINT8 IMR_Reg, UINT16* IMR_Result_Data);
R828_ErrCode R828_Section(R828_TUNER_NUM_TYPE R828_Tuner_Num, R828_SectType* SectionArry);
R828_ErrCode R828_F_IMR(R828_TUNER_NUM_TYPE R828_Tuner_Num, R828_SectType* IQ_Pont);
R828_ErrCode R828_IMR_Cross(R828_TUNER_NUM_TYPE R828_Tuner_Num, R828_SectType* IQ_Pont, UINT8* X_Direct);
...
R828_ErrCode R828_Filt_Cal(R828_TUNER_NUM_TYPE R828_Tuner_Num, UINT32 Cal_Freq,BW_Type R828_BW);
...
R828_ErrCode R828_Xtal_Check(R828_TUNER_NUM_TYPE R828_Tuner_Num)
{
	...
	if(I2C_Write(R828_Tuner_Num, &R828_I2C) != RT_Success)
	    return RT_Fail;
	...
	R828_Delay_MS(R828_Tuner_Num, 5);

	R828_I2C_Len.RegAddr = 0x00;
	R828_I2C_Len.Len     = 3;
	if(I2C_Read_Len(R828_Tuner_Num, &R828_I2C_Len) != RT_Success)
		return RT_Fail;
...
#endif
	   {
...
#endif 
	      {
...
#endif 
			  {
				   ...
				   //R828_Delay_MS(R828_Tuner_Num, 15);
				   R828_Delay_MS(R828_Tuner_Num, 20);
				   ...
				   {
						Xtal_cap_sel_tmp[R828_Tuner_Num] = XTAL_HIGH_CAP_0P;
				   }
			  }
		      else //0p lock
			  {
				   Xtal_cap_sel_tmp[R828_Tuner_Num] = XTAL_LOW_CAP_0P;
			  }
		   }
		   else //10p lock
		   {   
			   Xtal_cap_sel_tmp[R828_Tuner_Num] = XTAL_LOW_CAP_10P;
		   }
		}
		else //20p lock
		{
		   Xtal_cap_sel_tmp[R828_Tuner_Num] = XTAL_LOW_CAP_20P;
		}
	}
	else // 30p lock
	{
		Xtal_cap_sel_tmp[R828_Tuner_Num] = XTAL_LOW_CAP_30P;
	}
...
R828_ErrCode R828_Init(R828_TUNER_NUM_TYPE R828_Tuner_Num)
{
	...
	if(R828_IMR_done_flag[R828_Tuner_Num]==FALSE)
	{
          for (i=0; i<STD_SIZE; i++)
	      {	  
		      R828_Fil_Cal_flag[R828_Tuner_Num][i] = FALSE;
		      R828_Fil_Cal_code[R828_Tuner_Num][i] = 0;
	       }

		   for (i=0; i<5; i++)
	      {	
			  IMR_Data[R828_Tuner_Num][i].Gain_X = 0;
			  IMR_Data[R828_Tuner_Num][i].Phase_Y = 0;
			  IMR_Data[R828_Tuner_Num][i].Value = 0;
		   }
		  Xtal_cap_sel[R828_Tuner_Num] = XTAL_LOW_CAP_0P;
          Xtal_cap_sel_tmp[R828_Tuner_Num] = XTAL_LOW_CAP_0P;
	  //}

	  //write initial reg
	  if(R828_InitReg(R828_Tuner_Num) != RT_Success)        
		  return RT_Fail;

	  //Do Xtal check
	  if((Rafael_Chip==R820T) || (Rafael_Chip==R828S) || (Rafael_Chip==R820C))
	  {	 
		  Xtal_cap_sel[R828_Tuner_Num] = XTAL_HIGH_CAP_0P;
	  }
	  else
	  {
          if(R828_Xtal_Check(R828_Tuner_Num) != RT_Success)        //1st
	          return RT_Fail;

		  Xtal_cap_sel[R828_Tuner_Num] = Xtal_cap_sel_tmp[R828_Tuner_Num];
		  
		  if(R828_Xtal_Check(R828_Tuner_Num) != RT_Success)        //2nd
	          return RT_Fail;
		  
		  if(Xtal_cap_sel_tmp[R828_Tuner_Num] > Xtal_cap_sel[R828_Tuner_Num])
		  {
			  Xtal_cap_sel[R828_Tuner_Num] = Xtal_cap_sel_tmp[R828_Tuner_Num];
		  }

		  if(R828_Xtal_Check(R828_Tuner_Num) != RT_Success)        //3rd
	          return RT_Fail;
	  ...
	  //start imr cal.
	  if(R828_InitReg(R828_Tuner_Num) != RT_Success)        //write initial reg before doing cal
		return RT_Fail;

	  if(R828_IMR_Prepare(R828_Tuner_Num) != RT_Success)      
	      return RT_Fail;

	  if(R828_IMR(R828_Tuner_Num, 3, TRUE) != RT_Success)       //Full K node 3
		return RT_Fail;

	  if(R828_IMR(R828_Tuner_Num, 1, FALSE) != RT_Success)
		return RT_Fail;

	  if(R828_IMR(R828_Tuner_Num, 0, FALSE) != RT_Success)
		return RT_Fail;

	  if(R828_IMR(R828_Tuner_Num, 2, FALSE) != RT_Success)
		return RT_Fail;

	  if(R828_IMR(R828_Tuner_Num, 4, FALSE) != RT_Success)
		return RT_Fail;

	  R828_IMR_done_flag[R828_Tuner_Num] = TRUE;
	}
...
R828_ErrCode R828_InitReg(R828_TUNER_NUM_TYPE R828_Tuner_Num)
{
	...
	if(I2C_Write_Len(R828_Tuner_Num, &R828_I2C_Len) != RT_Success)
		return RT_Fail;
...
R828_ErrCode R828_IMR(R828_TUNER_NUM_TYPE R828_Tuner_Num, UINT8 IMR_MEM, BOOL IM_Flag)
{
	...
	//Must do before PLL() 
	if(R828_MUX(R828_Tuner_Num, (RingFreq - 5300)) != RT_Success)				//MUX input freq ~ RF_in Freq
		return RT_Fail;

	if(R828_PLL(R828_Tuner_Num, (RingFreq - 5300), STD_SIZE) != RT_Success)                //set pll freq = ring freq - 6M
	    return RT_Fail;

	if(IM_Flag == TRUE)
	{
	if(R828_IQ(R828_Tuner_Num, &IMR_POINT) != RT_Success)
		return RT_Fail;
	}
	else
	{
		IMR_POINT.Gain_X = IMR_Data[R828_Tuner_Num][3].Gain_X;
		IMR_POINT.Phase_Y = IMR_Data[R828_Tuner_Num][3].Phase_Y;
		IMR_POINT.Value = IMR_Data[R828_Tuner_Num][3].Value;
		if(R828_F_IMR(R828_Tuner_Num, &IMR_POINT) != RT_Success)
			return RT_Fail;
	}
	...
	{
	case 0:
		IMR_Data[R828_Tuner_Num][0].Gain_X  = IMR_POINT.Gain_X;
		IMR_Data[R828_Tuner_Num][0].Phase_Y = IMR_POINT.Phase_Y;
		IMR_Data[R828_Tuner_Num][0].Value   = IMR_POINT.Value;
		break;
	case 1:
		IMR_Data[R828_Tuner_Num][1].Gain_X  = IMR_POINT.Gain_X;
		IMR_Data[R828_Tuner_Num][1].Phase_Y = IMR_POINT.Phase_Y;
		IMR_Data[R828_Tuner_Num][1].Value   = IMR_POINT.Value;
		break;
	case 2:
		IMR_Data[R828_Tuner_Num][2].Gain_X  = IMR_POINT.Gain_X;
		IMR_Data[R828_Tuner_Num][2].Phase_Y = IMR_POINT.Phase_Y;
		IMR_Data[R828_Tuner_Num][2].Value   = IMR_POINT.Value;
		break;
	case 3:
		IMR_Data[R828_Tuner_Num][3].Gain_X  = IMR_POINT.Gain_X;
		IMR_Data[R828_Tuner_Num][3].Phase_Y = IMR_POINT.Phase_Y;
		IMR_Data[R828_Tuner_Num][3].Value   = IMR_POINT.Value;
		break;
	case 4:
		IMR_Data[R828_Tuner_Num][4].Gain_X  = IMR_POINT.Gain_X;
		IMR_Data[R828_Tuner_Num][4].Phase_Y = IMR_POINT.Phase_Y;
		IMR_Data[R828_Tuner_Num][4].Value   = IMR_POINT.Value;
		break;
    default:
		IMR_Data[R828_Tuner_Num][4].Gain_X  = IMR_POINT.Gain_X;
		IMR_Data[R828_Tuner_Num][4].Phase_Y = IMR_POINT.Phase_Y;
...
//	R828_Delay_MS(R828_Tuner_Num, 10);
...
{	
	...
	R828_I2C.RegAddr = 0x10;
	R828_Arry[11] &= 0xF4;
	switch(Xtal_cap_sel[R828_Tuner_Num])
	{
	...
	//Set_IMR
	if(R828_IMR_done_flag[R828_Tuner_Num] == TRUE)
	{
		RT_Reg08 = IMR_Data[R828_Tuner_Num][Freq_Info1.IMR_MEM].Gain_X & 0x3F;
		RT_Reg09 = IMR_Data[R828_Tuner_Num][Freq_Info1.IMR_MEM].Phase_Y & 0x3F;
	}
	else
	{
		RT_Reg08 = 0;
...
//	UINT8 IQ_Cunt  = 0;
	...
	for(VGA_Cunt = 12;VGA_Cunt < 16;VGA_Cunt ++)
	{
		...
		if(R828_Muti_Read(R828_Tuner_Num, 0x01, &VGA_Read) != RT_Success)
			return RT_Fail;
	...
	//{
	    // Determine X or Y
	    if(R828_IMR_Cross(R828_Tuner_Num, &Compare_IQ[0], &X_Direction) != RT_Success)
			return RT_Fail;
		...
		*/
		...
		{
			//compare and find min of 3 points. determine I/Q direction
		    if(R828_CompreCor(R828_Tuner_Num, &Compare_IQ[0]) != RT_Success)
			  return RT_Fail;

		    //increase step to find min value of this direction
		    if(R828_CompreStep(R828_Tuner_Num, &Compare_IQ[0], 0x08) != RT_Success)  //X
			  return RT_Fail;
		}
		else
		{
		   ...
		   //increase step to find min value of this direction
		   if(R828_CompreStep(R828_Tuner_Num, &Compare_IQ[0], 0x09) != RT_Success)  //Y
			 return RT_Fail;
		}
		/*
		//--- Y direction ---//
        ...
        */
		...
		if(X_Direction==1)
		{	    
           if(R828_IQ_Tree(R828_Tuner_Num, Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y, 0x08, &Compare_IQ[0]) != RT_Success) //Y
		     return RT_Fail;
		...
		{
		   if(R828_IQ_Tree(R828_Tuner_Num, Compare_IQ[0].Phase_Y, Compare_IQ[0].Gain_X, 0x09, &Compare_IQ[0]) != RT_Success) //X
		     return RT_Fail;
	...
	*/
	...
	if(R828_Section(R828_Tuner_Num, &Compare_IQ[0]) != RT_Success)
			return RT_Fail;
...
R828_ErrCode R828_IQ_Tree(R828_TUNER_NUM_TYPE R828_Tuner_Num, UINT8 FixPot, UINT8 FlucPot, UINT8 PotReg, R828_SectType* CompareTree)
{
	...
	for(TreeCunt = 0;TreeCunt < TreeTimes;TreeCunt ++)
	{
		...
		if(R828_Muti_Read(R828_Tuner_Num, 0x01, &CompareTree[TreeCunt].Value) != RT_Success)
			return RT_Fail;
...
R828_ErrCode R828_CompreStep(R828_TUNER_NUM_TYPE R828_Tuner_Num, R828_SectType* StepArry, UINT8 Pace)
{
	...
	while(((StepTemp.Gain_X & 0x1F) < IMR_TRIAL) && ((StepTemp.Phase_Y & 0x1F) < IMR_TRIAL))  //5->10
	{
		...
		if(R828_Muti_Read(R828_Tuner_Num, 0x01, &StepTemp.Value) != RT_Success)
			return RT_Fail;
...
// output: TRUE or FALSE
//-----------------------------------------------------------------------------------/
R828_ErrCode R828_Muti_Read(R828_TUNER_NUM_TYPE R828_Tuner_Num, UINT8 IMR_Reg, UINT16* IMR_Result_Data)  //jason modified
{
	UINT8 ReadCunt     = 0;
	UINT16 ReadAmount  = 0;
	UINT8 ReadMax = 0;
...
R828_ErrCode R828_Section(R828_TUNER_NUM_TYPE R828_Tuner_Num, R828_SectType* IQ_Pont)
{
	R828_SectType Compare_IQ[3];
	...
	if(R828_IQ_Tree(R828_Tuner_Num, Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y, 0x08, &Compare_IQ[0]) != RT_Success)  // y-direction
		return RT_Fail;
	...
	if(R828_IQ_Tree(R828_Tuner_Num, Compare_IQ[0].Gain_X, Compare_IQ[0].Phase_Y, 0x08, &Compare_IQ[0]) != RT_Success)
		return RT_Fail;
	...
	if(R828_CompreCor(R828_Tuner_Num, &Compare_Bet[0]) != RT_Success)
		return RT_Fail;
...
R828_ErrCode R828_IMR_Cross(R828_TUNER_NUM_TYPE R828_Tuner_Num, R828_SectType* IQ_Pont, UINT8* X_Direct)
{
	...
	for(CrossCount=0; CrossCount<5; CrossCount++)
	{
        ...
        if(R828_Muti_Read(R828_Tuner_Num, 0x01, &Compare_Cross[CrossCount].Value) != RT_Success)
		  return RT_Fail;
...
{	 
	...
	if(R828_IMR_done_flag[R828_Tuner_Num] == TRUE)
        R828_Arry[7]    = (R828_Arry[7] & 0xF0) | 0x01 | (Xtal_cap_sel[R828_Tuner_Num]<<1);
	else
...
/*
	...
	if(RT_Standard > SECAM_L1)
	{
		...
		//R828_Delay_MS(R828_Tuner_Num, 1);
	}
*/
	...
	// Filter Calibration
    if(R828_Fil_Cal_flag[R828_Tuner_Num][RT_Standard] == FALSE)
	{
		// do filter calibration 
		if(R828_Filt_Cal(R828_Tuner_Num, Sys_Info1.FILT_CAL_LO,Sys_Info1.BW) != RT_Success)
		    return RT_Fail;
		...
		R828_Fil_Cal_code[R828_Tuner_Num][RT_Standard] = R828_I2C_Len.Data[4] & 0x0F;

		//Filter Cali. Protection
		if((R828_Fil_Cal_code[R828_Tuner_Num][RT_Standard]==0) || (R828_Fil_Cal_code[R828_Tuner_Num][RT_Standard]==15))
		{
		   if(R828_Filt_Cal(R828_Tuner_Num, Sys_Info1.FILT_CAL_LO,Sys_Info1.BW) != RT_Success)
			   return RT_Fail;
		   ...
		   if(R828_Fil_Cal_code[R828_Tuner_Num][RT_Standard]==15) //narrowest
			   R828_Fil_Cal_code[R828_Tuner_Num][RT_Standard] = 0;
			   
		}
        R828_Fil_Cal_flag[R828_Tuner_Num][RT_Standard] = TRUE;
	}

	// Set Filter Q
	R828_Arry[5]  = (R828_Arry[5] & 0xE0) | Sys_Info1.FILT_Q | R828_Fil_Cal_code[R828_Tuner_Num][RT_Standard];  
	R828_I2C.RegAddr  = 0x0A;
	R828_I2C.Data     = R828_Arry[5];
	if(I2C_Write(R828_Tuner_Num, &R828_I2C) != RT_Success)
		return RT_Fail;
...
*/
	...
	if(R828_PLL(R828_Tuner_Num, Cal_Freq, STD_SIZE) != RT_Success)
		return RT_Fail;
...
R828_ErrCode R828_SetFrequency(R828_TUNER_NUM_TYPE R828_Tuner_Num, R828_Set_Info R828_INFO, R828_SetFreq_Type R828_SetFreqMode)
{
     ...
     if(R828_MUX(R828_Tuner_Num, LO_KHz) != RT_Success)
        return RT_Fail;



     //Set PLL
     if(R828_PLL(R828_Tuner_Num, LO_KHz, R828_INFO.R828_Standard) != RT_Success)
        return RT_Fail;
	 ...
	 if(R828_INFO.R828_Standard > SECAM_L1)
	 {
		 ...
		 else  //NORMAL mode
		 {
             ...
             */
			 ...
			 R828_Delay_MS(R828_Tuner_Num, 250);
			 /*
			 // PRE_DECT on
			 R828_Arry[1]  = (R828_Arry[1] & 0xBF) | SysFreq_Info1.PRE_DECT;
			 R828_I2C.RegAddr  = 0x06;
...
//	UINT8 IMR_Y[6] = {0, 0, 0, 0, 0, 0};
	...
	{
		R828_IMR_XT[IMR_C] = IMR_Data[0][IMR_C].Gain_X;
		R828_IMR_YT[IMR_C] = IMR_Data[0][IMR_C].Phase_Y;
	}
...
HI_S32 r820c_init_tuner(HI_U32 u32TunerPort)
{
    ...
    g_r820c_tuner_port[u32TunerPort] = u32TunerPort;

    //Initial R828
    if(R828_Init(g_r820c_tuner_port[u32TunerPort]) != RT_Success)
    {
        return HI_FAILURE;
    }
...
HI_S32 r820c_set_tuner(HI_U32 u32TunerPort, HI_U8 i2c_channel, HI_U32 puRF)
{
    ...
    if(R828_SetStandard(g_r820c_tuner_port[u32TunerPort], R828_Standard_Set) != RT_Success)
    {
        return HI_FAILURE;
    }
    ...
    if(R828_SetFrequency(g_r820c_tuner_port[u32TunerPort], R828_Info_Msg, R828_SetFreq_Mode) != RT_Success)
    {
        return HI_FAILURE;
    }
...
HI_VOID r820c_get_signal_strength(HI_U32 u32TunerPort, HI_U32 *pu32SignalStrength)
{
    ...
    err = R828_GetRfGain(g_r820c_tuner_port[u32TunerPort], &R828_rf_gain);
    if(RT_Success != err)
    {
...
{
    R828_IMR_done_flag[u32TunerPort] = FALSE;
    r820c_init_tuner(g_r820c_tuner_port[u32TunerPort]);
    return;
}
