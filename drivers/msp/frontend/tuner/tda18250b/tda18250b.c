...
#define situner_trace(level, msg...) do { \
		if((level) >= SITUNER_TRACE_LEVEL) { \
			HI_FATAL_TUNER(KERN_INFO "situner_trace:%s:%d: ", __FILE__, __LINE__); \
			HI_FATAL_TUNER(msg); \
			HI_FATAL_TUNER("\n"); \
		} \
	}while(0)
...
HI_S32 tda18250b_init_tuner(HI_U32 u32TunerPort)
{
    ...
    HI_INFO_TUNER("<0>func = %s, line = %d\n", __FUNCTION__, __LINE__);
    HI_INFO_TUNER("<0> TDA18250A_v%d.%d.%d\n", TDA18250A_COMP_NUM, TDA18250A_MAJOR_VER, TDA18250A_MINOR_VER);
    
    {
        ...
        if (err != TM_OK)
        {
            HI_INFO_TUNER("tmbslTDA18250A_Open return value 0x%x\n",err);
            /*goto FAIL;*/
        }
        ...
        if (err != TM_OK)
        {  
		HI_ERR_TUNER("tmbslTDA18250A_HwInit err 0x%x\n",err);
            /*goto FAIL;*/
        }
        ...
        if (err != TM_OK)
        {
           HI_ERR_TUNER("tmbslTDA18250A_SetPowerState err 0x%x\n",err);
            /*goto FAIL;*/
        }
        ...
        if (err != TM_OK)
        {
            HI_ERR_TUNER("tmbslTDA18250A_SetStandardMode err 0x%x\n",err);
...
HI_VOID tda18250b_tuner_resume(HI_U32 u32TunerPort)
{
    ...
    tda18250b_init_tuner(s_u32Tda18250ATunerPort);
