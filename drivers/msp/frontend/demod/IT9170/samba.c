...
unsigned long samba_init(void)
{
	...
	Byte var[2];
	/*----- read -----*/
	error = samba_rmr(0xEC86, 1, buf);		/*p_reg_p_tsm_init_clock_mode*/
	if (error) goto exit;
	error = samba_rmr(0xED03, 1, &buf[1]);	/*p_reg_p_fbc_n_code*/
	if (error) goto exit;
	error = samba_rmr(0xED23, 2, &buf[2]);	/*r_reg_r_fbc_m_bdry_7_0*/
	if (error) goto exit;
	...
	switch(g_clock_mode) {
		...
		/* patch for PeacockV4+Samba FPGA */
		case 1:	g_fxtal_kHz = 20480;	g_fdiv = 18;	val = 6;	break;	/*20.48MHz, 20480/32 = 640/1*/
	...
	cnt = 1;
	while (m_bdry == 0 && cnt < SAMBA_TUNER_INIT_POLL_COUNTS) {
		//Sleep(SAMBA_TUNER_INIT_POLL_INTERVAL);
		User_delay(Afatech_SAMBA_demodulator,SAMBA_TUNER_INIT_POLL_INTERVAL);
		error = samba_rmr(0xED23, 2, &buf[2]);	/*r_reg_r_fbc_m_bdry_7_0*/
		if (error) goto exit;
		m_bdry = (buf[3]<<8) + buf[2];	/*m_bdry = m_bdry_15_8 << 8 + m_bdry_7_0*/
		cnt++;
	...
	error = Standard_readRegister(Afatech_SAMBA_demodulator, Processor_LINK, chip_version_7_0, &chip_version);
	if (error) goto exit;
	error = Standard_readRegisters(Afatech_SAMBA_demodulator, Processor_LINK, chip_version_7_0+1, 2, var);
	if (error) goto exit;
	chip_Type = var[1]<<8 | var[0];	
	if(chip_Type==0x9175 && chip_version == 1){	
		User_delay(Afatech_SAMBA_demodulator,50);  //for samba
	}else{
		error = samba_rmr(0xEC82, 1, buf);	/*p_reg_p_tsm_init_mode*/
		if (error) goto exit;
		while (buf[0] == 0 && cnt < SAMBA_TUNER_INIT_POLL_COUNTS) {
			//Sleep(SAMBA_TUNER_INIT_POLL_INTERVAL);
			User_delay(Afatech_SAMBA_demodulator,SAMBA_TUNER_INIT_POLL_INTERVAL);
			error = samba_rmr(0xEC82, 1, buf);	/*p_reg_p_tsm_init_mode*/
			if (error) goto exit;
	...
	/*----- write -----*/
	error = samba_wmr(0xED81, 1, &val);		/*p_reg_p_iqik_m_cal*/
...
unsigned long samba_setfreq(unsigned int bw_kHz, unsigned int rf_freq_kHz)
{
	...
	/*----- read -----*/
	error = samba_rmr(0xEC4C, 1, buf);		/*p_reg_t_ctrl*/
	if (error) goto exit;
	error = samba_rmr(0xED81, 1, &buf[1]);	/*p_reg_p_iqik_m_cal*/
	if (error) goto exit;

	val[0] = samba_get_lna_cap_sel(rf_freq_kHz);
	val[1] = samba_get_lpf_bw(bw_kHz);
	...
	/*----- set_cal_freq -----*/
	tmp = samba_get_cal_freq_iqik(rf_freq_kHz, buf[1]);
	val[3] = (unsigned char) (tmp & 0xFF);
	val[4] = (unsigned char) ((tmp >> 8) & 0xFF);

	/*----- set_lo_freq -----*/
	tmp = samba_get_lo_freq(rf_freq_kHz);
	val[5] = (unsigned char) (tmp & 0xFF);
	val[6] = (unsigned char) ((tmp >> 8) & 0xFF);

	/*----- write -----*/
	error = Standard_writeRegisters (Afatech_SAMBA_demodulator, Processor_OFDM, var_pre_lna_cap_sel, 1, val);
	if (error) goto exit;
	
	error = samba_wmr(0xEC56, 1, &val[1]);	/*p_reg_t_lpf_bw*/
	if (error) goto exit;
	error = samba_wmr(0xEC4C, 1, &val[2]);	/*p_reg_t_ctrl*/
	if (error) goto exit;
	/*----- the writing sequence is important for cal_freq and lo_freq, thus separate ----- */
	error = samba_wmr(0xEC4D, 1, &val[3]);	/*p_reg_t_cal_freq_7_0*/
	if (error) goto exit;
	error = samba_wmr(0xEC4E, 1, &val[4]);	/*p_reg_t_cal_freq_15_8*/
	if (error) goto exit;

	error = Standard_writeRegisters (Afatech_SAMBA_demodulator, Processor_OFDM, var_pre_lo_freq_7_0, 1, &val[5]);
	if (error) goto exit;

	error = Standard_writeRegisters (Afatech_SAMBA_demodulator, Processor_OFDM, var_pre_lo_freq_15_8, 1, &val[6]);
exit:
	return (error);
}

/*samba_rmr (_read _multiple _registers)*/
unsigned long samba_rmr(unsigned long mem_addr, unsigned char len, unsigned char *buf)
{	
	return (Standard_readRegisters (
		Afatech_SAMBA_demodulator, Processor_OFDM, mem_addr, len, buf));
}
/*samba_wmr (_write _multiple _registers)*/
unsigned long samba_wmr(unsigned long mem_addr, unsigned char len, unsigned char *val)
{
	return (Standard_writeRegisters (
		Afatech_SAMBA_demodulator, Processor_OFDM, mem_addr, len, val));
}
...
unsigned int samba_get_cal_freq_iqik(unsigned int rf_freq_kHz, unsigned char iqik_m_cal)
{
	...
	cal_freq = samba_get_freq_code(rf_freq_kHz, &nc, &nv, &mv);
	if (g_clock_mode==0 && (iqik_m_cal < SAMBA_IQIK_M_CAL_MID)) {
		mv = mv + ((((unsigned int)iqik_m_cal)*nv*9)>>5);
	}
	else if (g_clock_mode==0 && (iqik_m_cal >= SAMBA_IQIK_M_CAL_MID)) {
		iqik_m_cal = SAMBA_IQIK_M_CAL_MAX - iqik_m_cal;
		mv = mv - ((((unsigned int)iqik_m_cal)*nv*9)>>5);
	}
	else if (g_clock_mode==1 && (iqik_m_cal < SAMBA_IQIK_M_CAL_MID)) {
		mv = mv + ((((unsigned int)iqik_m_cal)*nv)>>1);
	}
	else {	/*(g_clock_mode==1 && (iqik_m_cal >= SAMBA_IQIK_M_CAL_MID))*/
		iqik_m_cal = SAMBA_IQIK_M_CAL_MAX - iqik_m_cal;
		mv = mv - ((((unsigned int)iqik_m_cal)*nv)>>1);
	}
...
unsigned int samba_get_lo_freq(unsigned int rf_freq_kHz)
{
	...
	lo_freq = samba_get_freq_code(rf_freq_kHz, &nc, &nv, &mv);
	return lo_freq;
}
...
unsigned long samba_get_param(SAMBA_Param param, int *int_buf)
{
	unsigned long error = 0;
...
unsigned long samba_set_param(SAMBA_Param param, int *int_val)
{
	unsigned long error = 0;
	//unsigned char val[4];
	//double rf_gain, total_gain;
