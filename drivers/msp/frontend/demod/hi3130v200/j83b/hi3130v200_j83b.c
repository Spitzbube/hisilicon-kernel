...
static REGVALUE_S s_stHi3130j83bHardReg[]=
{
    #include "qamdata/Hi3130V200_HARD_J83B.txt"
};

static HI_VOID hi3130v200_j83b_select_qam_block(HI_U32 u32TunerPort)
{
    HI_U8 u8Tmp = 0;
...
static char *s_stHi3130J83BQamRegName[]=
{
...
static HI_VOID set_symrate(HI_U32 u32TunerPort, HI_U32 u32SymRate)
{
    ...
    if((u8VersionReg >> 5) == 5)
    {
        hi3130v200_j83b_select_qam_block(u32TunerPort);
    }
...
HI_VOID hi3130j83b_set_hard_reg_value(HI_U32 u32TunerPort)
{
    HI_S32  i = 0; 
    HI_S32	s32RegSum; 
    REGVALUE_S *pstReg;
    ...
    pstReg = s_stHi3130j83bHardReg;
  
    s32RegSum = sizeof(s_stHi3130j83bHardReg)/sizeof(REGVALUE_S);
...
HI_VOID hi3130j83b_test_single_agc( HI_U32 u32TunerPort, AGC_TEST_S * pstAgcTest )
{
    HI_S32 s32Ret = 0;
    HI_U8  u8RegVal = 0;
    HI_U8  au8Tmp[2] = {0};
...
static HI_VOID hi3130j83b_chip_reset(HI_U32 u32TunerPort, qam_module_e enResetModule)
{
    HI_U8 u8VersionReg = 0;
...
HI_VOID hi3130j83b_get_registers(HI_U32 u32TunerPort, void *p)
{
	HI_U8   u8Value = 0;
	HI_S32  i = 0; 
	HI_S32	s32RegSum = 0;
	...
	for( i = 0; i < s32RegSum; i++ )
	{
	    ...
	    pstReg++;
	    PROC_PRINT(p, "{%s  ,0x%02x },  ", s_stHi3130J83BQamRegName[i], u8Value);
	}
	PROC_PRINT(p, "\n\n");

}

static HI_VOID hi3130j83b_set_customreg_value(HI_U32 u32TunerPort, HI_BOOL bInversion, HI_U32 u32SymbolRate)
{
...
static HI_VOID hi3130j83b_set_reg_value(HI_U32 u32TunerPort)
{
    HI_S32  i = 0; 
	HI_S32	s32RegSum; 
	HI_U32	u32QamType;
   ...
   switch( g_stTunerOps[u32TunerPort].enTunerDevType )
   {
    ...
    for(i=0; i<s32RegSum; i++)
    {
     ...
     if((0!=u32TunerPort) && (HI_UNF_TUNER_DEV_TYPE_TDA18250B == g_stTunerOps[u32TunerPort].enTunerDevType))
    {
        //plus 18250B IF:3.5+0.25M
            qam_write_byte(u32TunerPort, FS_CTRL_1_ADDR, 0x04);   
            qam_write_byte(u32TunerPort, FS_CTRL_2_ADDR, 0xf8);  
      
	 
    }
}

HI_S32 hi3130j83b_get_status (HI_U32 u32TunerPort, HI_UNF_TUNER_LOCK_STATUS_E  *penTunerStatus)
{
    HI_U8  u8Status = 0;
...
HI_VOID hi3130j83b_manage_after_chipreset(HI_U32 u32TunerPort)
{
    HI_S32  s32Cnt = 0;
...
static HI_U32 s_u32Hi3130J83bConnectTimeout = HI3130J83B_CONNCECT_TIMEOUT;
HI_VOID hi3130j83b_connect_timeout(HI_U32 u32ConnectTimeout)
{
    s_u32Hi3130J83bConnectTimeout = u32ConnectTimeout;
    return;
}
HI_S32 hi3130j83b_connect(HI_U32 u32TunerPort, TUNER_ACC_QAM_PARAMS_S *pstChannel)
{
 ...
 //   HI_U8 u8VersionReg = 0;
    ...
    hi3130j83b_set_reg_value(u32TunerPort);    /*set for qam and tunner*/

    /*set for different usr request about symbolrate, ts type, demo in or out,  inversion or not*/
    hi3130j83b_set_customreg_value(u32TunerPort, pstChannel->bSI, u32SymbolRate);

    /*step 3:reset chip*/
    hi3130j83b_chip_reset(u32TunerPort, ALL_CHIP);

    if(s_u32Hi3130J83bConnectTimeout < HI3130J83B_CLOSE_EQU_TIME)
    {
        s_u32Hi3130J83bConnectTimeout = HI3130J83B_CONNCECT_TIMEOUT;
        return HI_SUCCESS;
    }
#if 0
...
HI_S32 hi3130j83b_get_signal_strength(HI_U32 u32TunerPort, HI_U32 *pu32SignalStrength)
{
    HI_U8  au8RegVal[2] = {0};
...
HI_S32 hi3130j83b_get_ber(HI_U32 u32TunerPort, HI_U32 *pu32ber)
{
...
HI_S32 hi3130j83b_get_rs(HI_U32 u32TunerPort, HI_U32 *pu32Rs)
{
    HI_U32 u32BerThres = 4; /* modify by chenxu 2008-01-15 AI7D02092 */
    HI_U8  u8Temp    = 0;
    HI_U8  u8Val     = 0;
...
HI_S32 hi3130j83b_get_snr(HI_U32 u32TunerPort, HI_U32* pu32SNR)
{
    HI_U32 u32Snr = 0;
    HI_U8  u8RdVal = 0;
    HI_U32 i ;
...
HI_S32 hi3130j83b_set_ts_type(HI_U32 u32TunerPort, HI_UNF_TUNER_OUPUT_MODE_E enTsType)
{
...
HI_S32 hi3130j83b_get_freq_symb_offset(HI_U32 u32TunerPort, HI_U32 * pu32Freq, HI_U32 * pu32Symb )
{
    HI_U8  au8Freq[4] = { 0 };
...
static HI_U8 s_stHi3130j83bIOShare[MAX_TS_LINE][MAX_TS_LINE]=
{
...
HI_S32 hi3130j83b_set_ts_out(HI_U32 u32TunerPort, HI_UNF_TUNER_TSOUT_SET_S *pstTSOut)
{
    HI_U8 u8Temp= 0;
    HI_U32	i = 0;
    ...
    for (i = 0; i < MAX_TS_LINE; i++)
    {
	 ...
	 switch(i)
	 {
		  case 0:		  				
		  	...
		  	u8Temp = (u8Temp & 0xf0)  | s_stHi3130j83bIOShare[i][pstTSOut->enTSOutput[i]];
			qam_write_byte(u32TunerPort, IOSHARE_CTRL_4_ADDR, u8Temp);
		  	break;
		 case 1:
		       qam_read_byte(u32TunerPort, IOSHARE_CTRL_4_ADDR, &u8Temp);	   
		  	u8Temp = (u8Temp & 0x0f)  | (s_stHi3130j83bIOShare[i][pstTSOut->enTSOutput[i]] <<4);
			qam_write_byte(u32TunerPort, IOSHARE_CTRL_4_ADDR, u8Temp);
		  	break;
		 case 2:
