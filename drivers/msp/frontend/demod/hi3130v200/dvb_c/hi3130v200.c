...
//static HI_VOID hi3130v200_select_hard_ip_block(HI_U32 u32TunerPort);
   ...
   { 
    ...
    if((0!=u32TunerPort) && (HI_UNF_TUNER_DEV_TYPE_TDA18250B == g_stTunerOps[u32TunerPort].enTunerDevType))
    {
        ...
        if(4 == u32QamType)
        {
            ...
            qam_write_byte(u32TunerPort, FS_CTRL_2_ADDR, 0xc8);  
        }
	    else
	    {
	        qam_write_byte(u32TunerPort, FS_CTRL_1_ADDR, 0x15);   
            qam_write_byte(u32TunerPort, FS_CTRL_2_ADDR, 0xd4);
	     }
