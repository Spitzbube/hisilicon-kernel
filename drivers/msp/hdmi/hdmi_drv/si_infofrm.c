...
void SI_SendAVIInfoFrame(void)
{
    ...
    {
        HI_ERR_HDMI(" Send AVI Err:0x%x, En not Zero! :0x%x\n", RegVal, ReadByteHDMITXP1(INF_CTRL1));
        //return;
    }
#endif
    DelayMS(5);
    ...
    HI_INFO_HDMI("Read AVI Data\n");
    for(i = 0; i < 13; i++ )
    {
        RegVal = SI_ReadByteEEPROM ( EEAddr++);
        WriteByteHDMITXP1( TXAddr++ , RegVal);
    }
    HI_INFO_HDMI("open AVI packet en/repeat\n");
    RegVal = ReadByteHDMITXP1( INF_CTRL1);
    HI_INFO_HDMI("open AVI packet en/repeat2\n");
    WriteByteHDMITXP1(INF_CTRL1, RegVal | ( BIT_AVI_ENABLE | BIT_AVI_REPEAT));
    HI_INFO_HDMI("open AVI packet en/repeat3\n");
    RegVal = ReadByteHDMITXP1( INF_CTRL1);
    ...
    HI_INFO_HDMI("End of AVI inforframe\n");
#endif
    return;
}
