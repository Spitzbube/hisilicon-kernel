...
static HI_U8 GetByteBStatusMX(HI_U8 Addr)
{
    ...
    if(Addr <2 )
    {
        ...
        HI_INFO_HDMI("BSTATUS:0x%x,0x%x\n", Data[0], Data[1]);
        return Data[Addr];
    }
    else if(Addr < 10)
    {
...
static HI_U8 GetKSVList(SHA_CTX IRAM * Sha, HI_U16 i)
{
    ...
    }            
    {
        ...
        {    
            HDMI_DEBUG("0x%x, ", Sha->Data[index&0xf]);
        }
        HDMI_DEBUG("\n");
        HDMI_DEBUG("Byte sequence in Sha->Data is:\n");
        tmppoint = (HI_U8 *)&(Sha->Data[0]);
        for (index = 0; index < ((Aux+1) * 4); index ++)
        {
            HDMI_DEBUG("[%d]:0x%x, ", index, tmppoint[index&0x3f]);
        }
        HDMI_DEBUG("\n");
...
static Bool CompareVi(SHA_CTX IRAM * Sha)
{
...
#endif
        HI_INFO_HDMI("Sha->Digest[%d]:0x%x, VPrime:0x%x\n", i, Sha->Digest[i], VPrime);
    ...
    HI_INFO_HDMI("CompareVi is Result 0x%x *****\n", Result);
...
void SI_SHAHandler(void)
{
    ...
    {
        HI_INFO_HDMI("Repeater:enter hdcp protocol 2 REQ_SHA_CALC\n");
        ...
        HI_INFO_HDMI("KSVSize(DEVICE_COUNT*5):%d\n", KSVSize);

        if (KSVSize)
        {
            HI_INFO_HDMI("read KSV List\n");
            SI_MDDCInitReadBlockFromFIFO(DDC_KSV_FIFO_ADDR, KSVSize );
        }
        
        SHAInit(&sha);
        HI_INFO_HDMI("After SHAInit sha->Digest:0x%x, 0x%x, 0x%x, 0x%x\n", sha.Digest[0], sha.Digest[1], sha.Digest[2], sha.Digest[3]);
        ...
        if(CompareVi(&sha))
        {
            ...
            HI_INFO_HDMI("SI_SHAHandler ok, enter hdcp protocol 3\n");
            SI_timer_count();
        ...
        {
        	HI_ERR_HDMI("SI_SHAHandler(hdcp protocol 2) fail\n");
