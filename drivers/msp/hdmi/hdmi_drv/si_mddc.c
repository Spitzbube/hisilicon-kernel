...
HI_U8 SI_BlockWriteMDDCFifo(HI_U8 ucLen, HI_U8 *pucData)
{
    ...
    HI_INFO_HDMI("Write MDDC_FIFO ucLen=%d \n", ucLen);
...
HI_U8 SI_BlockRead_MDDC(MDDCType * MDDCCmd)
{
    ...
    {
        HI_ERR_HDMI("DDC MASTER_BASE Write Error=%d\n", Err);
        goto FAILE_RETURN;
    }
    //SI_DDC_Debug();
    ...
    if(Status)
    {
        ...
        HI_INFO_HDMI("%s ConvertMDDCStateIntoErrCode Error=%d\n", 
            __FUNCTION__, Status);
        goto FAILE_RETURN;
    }
...
HI_U8 SI_BlockWrite_MDDC(MDDCType * MDDCCmd)
{
    ...
    if(IIC_OK == tmp.Err)
    {
        ...
        if(IIC_OK != tmp.Err)
        {
            HI_ERR_HDMI("%s Write DDC fifo Error=%d\n", __FUNCTION__, tmp.Err);
        }
    }
    else
    {
        HI_ERR_HDMI("%s Write DDC MASTER_BASE Error=%d\n", __FUNCTION__, tmp.Err);
    }
    /* After transfer, check DDC status */
    if(IIC_OK != tmp.Err)
    {
        ...
        {
           mdelay(1);//msleep(1); //wait data has been written in ready
        }
