...
HI_U8 AreR0sMatch(HI_U8 * Match)
{
    ...
    if(!Error) 
    {
        ...
        //HI_INFO_HDMI("AreR0sMatch:R0RX:0x%x should == R0TX:0x%x\n", R0RX, R0TX);
        if(R0RX == R0TX)
              *Match = TRUE;
        else
              *Match = FALSE;
...
static void MakeCopyM0(void)
{
    ...
    HI_INFO_HDMI("M0 MX_TX :0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x\n", \
...
void SI_AutoRiCheck ( Bool qOn )
{
    HI_INFO_HDMI("SI_AutoRiCheck ON=%d\n", qOn);
...
void SI_AutoRiFailureHandler( HI_U8 bRiFIntr )
{
    if (bRiFIntr & MASK_AUTO_RI_9134_SPECIFIC)
    {
        ...
        HI_INFO_HDMI ("Ri interrupt asserted %d times\n", ++RiFailureCount);
...
HI_U8 SI_ReadKSVList_Timeout(void)
{
    ...
    //HI_INFO_HDMI("hdcp interval is :%x s.%x us\n", (HI_U32)(TimeSKVList.tv_sec - TimeR0.tv_sec), (HI_U32)(TimeSKVList.tv_usec - TimeR0.tv_usec));
    return HI_FALSE;
}
#else
...
HI_U8 SI_ReadKSVList_Timeout(void)
{
    ...
    //HI_INFO_HDMI("hdcp interval is :%d ms\n", (HI_U32)(jiffies_now - jiffies_org));
    return HI_FALSE;
}
#endif
...
HI_U8 SI_Authentication( void )
{
    ...
    if (HI_TRUE == SI_Is_HDCPAuth_DetectHPD())
    {
        ...
        HI_WARN_HDMI("Need to reAuthentication 1\n");
        DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
    ...
    if(Err)
    {
        HI_WARN_HDMI("%s SI_CheckHDCPDevice ERROR\n", __FUNCTION__);
        DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
    ...
    if(!tmp.HDCPDev)
    {
        HI_WARN_HDMI("%s NO_DEVICE_WITH_HDCP_SLAVE_ADDR\n", __FUNCTION__);
        DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
...
*/
    HI_INFO_HDMI("new setting to create HDCP Auth1\n");
    if (HI_TRUE == SI_Is_HDCPAuth_DetectHPD())
    {
        //High priority to deal with HPD!, before communciantion!!
        HI_WARN_HDMI("Need to reAuthentication 2\n");
        DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
        return REAUTHENTATION_REQ;
    }
    ...
    if(!Err) 
    {
        ...
        {
            //High priority to deal with HPD!, before communciantion!!
            HI_WARN_HDMI("Need to reAuthentication 3\n");
            DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
            return REAUTHENTATION_REQ;
        }
        ...
        {
            //High priority to deal with HPD!, before communciantion!!
            HI_WARN_HDMI("Need to reAuthentication 4\n");
            DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
            return REAUTHENTATION_REQ;
        }
        ...
        if(!Err)
        {
            ...
            {
                //High priority to deal with HPD!, before communciantion!!
                HI_WARN_HDMI("Need to reAuthentication 5\n");
                DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
                return REAUTHENTATION_REQ;
            }
            ...
            if (SI_CheckHDCPAKSV(AN_KSV_Data) != HI_TRUE)
     	    {
     	    	HI_WARN_HDMI("AKSV 20 zeros\n");
      	    	DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
                return NO_ACK_FROM_HDCP_DEV;
    	    }
            Err = MDDCBlockReadHDCPRX(5, DDC_BKSV_ADDR , AN_KSV_Data); // read BKSV from RX            

            HI_INFO_HDMI("\nHDCP BKSV(5):0x%02x,0x%02x,0x%02x,0x%02x,0x%02x\n", \
                AN_KSV_Data[0],AN_KSV_Data[1],AN_KSV_Data[2],AN_KSV_Data[3],AN_KSV_Data[4]);

            if (0 != Err)
            {
                HI_ERR_HDMI("MDDCBlockReadHDCPRX DDC_BKSV_ADDR ERROR\n");
            }
        }
        else
        {
            HI_ERR_HDMI("MDDCBlockWriteHDCPRX DDC_AKSV_ADDR ERROR\n");
        }
   }
   else
   {
        HI_ERR_HDMI("MDDCBlockWriteHDCPRX DDC_AN_ADDR ERROR\n");
   }
   
    if(0 != Err)
    {
        HI_WARN_HDMI("NO_ACK_FROM_HDCP_DEV\n");
        DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
        return NO_ACK_FROM_HDCP_DEV;
    }
    
    HI_INFO_HDMI("Exchange KSVs ok.\n");
    
    BlockWriteHDCPTX(5, BKSV_ADDR, AN_KSV_Data); // Write BKSV into TX
    if(IsBKSVError())
    {
        HI_WARN_HDMI("BKSV_ERROR\n");
        DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
        return BKSV_ERROR;
    }
    ...
    {
        //High priority to deal with HPD!, before communciantion!!
        HI_WARN_HDMI("Need to reAuthentication 6\n");
        DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
        return REAUTHENTATION_REQ;
    }  
    ...
    {
        //High priority to deal with HPD!, before communciantion!!
        HI_INFO_HDMI("Need to reAuthentication 7\n");
        return REAUTHENTATION_REQ;
    }
#else
{
	...
	for(index = 0; index < 10; index ++)
	{
	    ...
	    {
	        //High priority to deal with HPD!, before communciantion!!
	        HI_WARN_HDMI("Need to reAuthentication 8\n");
            DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
	        return REAUTHENTATION_REQ;
	    }
	}
    ...
    //HI_INFO_HDMI("Protocol step 1:come to compare R0 and R0`\n");
    Err = AreR0sMatch(&tmp.R0sMatch);
    if(!Err)
    {
         if(!tmp.R0sMatch)
         {
             HI_WARN_HDMI("R0s_ARE_MISSMATCH.\n");
             DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
    ...
    {
        HI_WARN_HDMI("AreR0sMatch Error:NO_ACK_FROM_HDCP_DEV\n");
        DRV_HDMI_NotifyEvent(HI_UNF_HDMI_EVENT_HDCP_FAIL);
        return NO_ACK_FROM_HDCP_DEV;
    }

    //HI_INFO_HDMI("Protocol step 1 sucess!\n");
    if(IsRepeater()) 
    {
        MakeCopyM0();
        SI_SetEncryption(ON);
...
#else
        jiffies_org = jiffies;
        HI_INFO_HDMI("5s is %d jiffers\n", (HI_U32)msecs_to_jiffies(5000));
#endif        
        DEBUG_PRINTK("Enter Protocol step 2:repeater authetication\n");
        SI_timer_count();
...
void SI_ReAthentication( void )
{
    ...
    if(!SI_RSEN_Status())
    {
    	DEBUG_PRINTK("Sink is in standy mode\n");
    ...
    {
    	HI_INFO_HDMI("RSEN is power\n");
    }

    HI_INFO_HDMI("SI_ReAthentication AuthState:%d\n", AuthState);
    SI_timer_count();
    ...
    {
        Mode = _DVI_;
        HI_INFO_HDMI ("Output is DVI mode, Call SI_Init_DVITX()\n");
    ...
    */
    ...
    else
    {
     ...
     if((AuthState == AUTHENTICATED)||(AuthState == REPEATER_AUTH_REQ)) 
     {
        ...
        {
            SI_timer_count();
            HI_INFO_HDMI("HDMI receive is sink device, enter hdcp protocol 3\n");
    ...
    {
    	HI_INFO_HDMI("line:%d:AuthState:0x%x\n",__LINE__,AuthState);
    	SI_SendCP_Packet(ON);
    }
