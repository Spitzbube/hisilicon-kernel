...
static void SetDS_BStatus ( HI_U8 * pbData ){
    ...
    WriteBlockHDMITXP0(DS_BSTATUS_ADDR, 2, pbData );
}
//------------------------------------------------------------------------------
...
HI_U8 bData[5], i;
    ...
    ClearKSVFIFOPointer();
    MDDCInitReadBlockFromFIFO( DDC_KSV_FIFO_ADDR, bNumberDS_Devices * 5 );

    for (i = 0; i < bNumberDS_Devices; i++ ){
        MDDCTakeBlockFromFIFO( 5, bData );
        WriteBlockHDMITXP0( KSV_FIFO_ADDR, 5, bData );
    }
...
HI_U8 i, j;
    ...
    for(i = 0; i < 5; i ++) {
        MDDCTakeBlockFromFIFO( 4, (HI_U8 *)&bVPrime);
        ReadBlockHDMITXP0( V_ADDR + i * 4, 4, bV );
        for ( j = 0; j < 4; j++){
            if ( bVPrime[j] != bV[j] ){
                qResult = FALSE;
                break;
...
Bool qResult = TRUE;
    ...
    {
        HI_INFO_HDMI("-->AuthState == REQ_SHA_HW_CALC.\n");
