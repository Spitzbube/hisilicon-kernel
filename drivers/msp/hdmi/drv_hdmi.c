...
//    DISP_HDMI_SETTING_S stDispCfg;
    ...
    if (bOpenAlready == HI_TRUE)
    {
        ...
        {
            HI_U32 bHdmiMode;
            /*if fastboot is TV format,setting VGA format *//*CNcomment:fastboot 是tv， 将要设置为vga*/
            ...
            else if (!bHdmiMode /*if fastboot is VGA format, setting TV format*//*CNcomment:fastboot 是vga， 将要设置为tv*/
                    &&(HI_DRV_DISP_FMT_861D_640X480_60 >= enEncFmt))            
            {
                bOpenAlready = HI_FALSE;
            }
            else if (!bHdmiMode /*if fastboot is VGA format, setting VGA format *//*CNcomment:fastboot 是vga， 将要设置为vga*/
                    &&(HI_DRV_DISP_FMT_861D_640X480_60 < enEncFmt))
            {
                bOpenAlready = HI_TRUE;
            }
...
#endif
        #else
        //原有方案，多进程多线程下，Event[0]位置 时不时丢失消息
